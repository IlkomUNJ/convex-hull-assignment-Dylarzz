...
DrawingCanvas::DrawingCanvas(QWidget *parent) : QWidget(parent)
{
    ...
    m_iterationCount = 0; // Initialize iteration count
}
...
void DrawingCanvas::paintEvent(QPaintEvent *event)
{
    ...
    // Draw the text for point and iteration counts
    painter.setPen(Qt::black);
    painter.setFont(QFont("Arial", 12));
    QString infoText = QString("Points: %1").arg(m_points.size());
    if (m_drawConvexHull) {
        infoText.append(QString(" | Iterations: %1").arg(m_iterationCount));
    }
...
void DrawingCanvas::mousePressEvent(QMouseEvent *event)
{
    ...
    // Reset the convex hull flag and counters
    m_drawConvexHull = false;
...
{
    m_iterationCount = 0; // Reset counter
    if (m_points.size() < 3) {
    ...
    for (const auto& p : sortedPoints) {
        while (upperHull.size() >= 2) {
            m_iterationCount++; // Count each orientation check
            if (orientation(upperHull[upperHull.size() - 2], upperHull.back(), p) != 2) {
                upperHull.pop_back();
            } else {
                break;
            }
    ...
    for (int i = sortedPoints.size() - 1; i >= 0; --i) {
        ...
        while (lowerHull.size() >= 2) {
            m_iterationCount++; // Count each orientation check
            if (orientation(lowerHull[lowerHull.size() - 2], lowerHull.back(), p) != 2) {
                lowerHull.pop_back();
            } else {
                break;
            }
    ...
    if (!lowerHull.isEmpty()) lowerHull.pop_back();
    if (!lowerHull.isEmpty()) lowerHull.pop_front();
...
{
    // DIGANTI: Menggunakan m_iterationCount
    m_iterationCount = 0;
...
void DrawingCanvas::clearCanvas()
{
    ...
    m_convexHullPoints.clear();
    m_drawConvexHull = false;
    m_iterationCount = 0; // Reset iteration count
    update();
}

int DrawingCanvas::orientation(QPoint p, QPoint q, QPoint r)
{
    qint64 val = (qint64)(q.y() - p.y()) * (r.x() - q.x()) - (qint64)(q.x() - p.x()) * (r.y() - q.y());
    if (val == 0) return 0;
    return (val < 0) ? 1 : 2;
}
